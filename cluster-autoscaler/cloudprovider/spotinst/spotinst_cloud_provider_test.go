...
var testCloudManager = &CloudManager{
...
func testCloudProvider(t *testing.T, m *CloudManager) *CloudProvider {
	cloud, err := BuildCloudProvider(m, nil)
	assert.NoError(t, err)
	return cloud
}

func TestBuildCloudProvider(t *testing.T) {
	_, err := BuildCloudProvider(testCloudManager, []string{"bad spec"})
	assert.Error(t, err)

	_, err = BuildCloudProvider(testCloudManager, nil)
	assert.NoError(t, err)
}

func TestAddNodeGroup(t *testing.T) {
	provider := testCloudProvider(t, testCloudManager)
	err := provider.addNodeGroup("bad spec")
	assert.Error(t, err)
	assert.Equal(t, len(provider.groups), 0)

	err = provider.addNodeGroup("1:5:sig-test")
	assert.NoError(t, err)
	assert.Equal(t, len(provider.groups), 1)
}
...
func TestMaxSize(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, len(provider.groups), 1)
	assert.Equal(t, provider.groups[0].MaxSize(), 5)
}
...
func TestMinSize(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, len(provider.groups), 1)
	assert.Equal(t, provider.groups[0].MinSize(), 1)
}
...
func TestTargetSize(t *testing.T) {
	...
	err := provider.addNodeGroup("1:5:sig-test")
	assert.NoError(t, err)
	targetSize, err := provider.groups[0].TargetSize()
...
func TestIncreaseSize(t *testing.T) {
	...
	cloud := testCloudManager.groupService.CloudProviderAWS().(*awsServiceMock)
	cloud.On("Update", context.Background(), &aws.UpdateGroupInput{
		Group: &aws.Group{
			ID: spotinst.String(provider.groups[0].Id()),
			Capacity: &aws.Capacity{
				Target:  spotinst.Int(3),
				Minimum: spotinst.Int(provider.groups[0].minSize),
				Maximum: spotinst.Int(provider.groups[0].maxSize),
			},
		},
	}).Return(&aws.UpdateGroupOutput{})

	err = provider.groups[0].IncreaseSize(1)
	assert.NoError(t, err)
...
func TestBelongs(t *testing.T) {
	...
	_, err = provider.groups[0].Belongs(invalidNode)
	assert.Error(t, err)
	...
	belongs, err := provider.groups[0].Belongs(validNode)
...
func TestDeleteNodes(t *testing.T) {
	...
	cloud.On("Detach", context.Background(), &aws.DetachGroupInput{
		GroupID:                       spotinst.String(provider.groups[0].Id()),
	...
	err = provider.groups[0].DeleteNodes([]*apiv1.Node{node})
	assert.NoError(t, err)
	cloud.AssertExpectations(t)
}
...
func TestId(t *testing.T) {
	...
	assert.NoError(t, err)
	assert.Equal(t, len(provider.groups), 1)
	assert.Equal(t, provider.groups[0].Id(), "sig-test")
}

func TestDebug(t *testing.T) {
	grp := Group{
		manager: testCloudManager,
		minSize: 5,
		maxSize: 55,
	}
...
func TestBuildGroup(t *testing.T) {
	...
	_, err := provider.buildGroupFromSpec("a")
	assert.Error(t, err)
	_, err = provider.buildGroupFromSpec("a:b:c")
	assert.Error(t, err)
	_, err = provider.buildGroupFromSpec("1:")
	assert.Error(t, err)
	_, err = provider.buildGroupFromSpec("1:2:")
	assert.Error(t, err)

	grp, err := provider.buildGroupFromSpec("111:222:sig-test")
	assert.NoError(t, err)
